...
# 0.125-0.145s against a 0.05s budget. Constructing and closing the
# `httpx.AsyncClient` accounts for about 0.078s of that — more than the budget
# itself — so most of what the test used to measure was setup, not deadline.
# The old bound of 0.5s left 0.372s of absolute headroom, and CI jitter is
# absolute, not proportional: in swiss-efv-mcp a loaded runner turned 0.105s
# into 0.55s on 2026-08-21 and tore the same assertion there. Raising the
# budget does not shrink that stall, it makes the stall small *relative to*
# what is measured.
_BUDGET = 0.5
_CUT_BY = 2.5
_SLOW_RESPONSE = 8.0
...
async def test_a_slow_response_is_cut_by_the_wall_clock_deadline():
    ...
    async with httpx.AsyncClient() as http:
        ...
        with pytest.raises(TimeoutError):
            await _call(http, total_budget=_BUDGET)
