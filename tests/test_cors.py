...
Freigabeliste hier nannte nur `Content-Type` und `Mcp-Session-Id` — den Session-Header, der fuer sich genommen keine Anfrage routet.

Ein Browser darf einen nicht safelisteten Header aber gar nicht erst senden,
wenn der Server ihn nicht in `Access-Control-Allow-Headers` nennt. Der
Preflight endete mit 400, und zwar bevor irgendein MCP-Byte floss. stdio- und
Python-Clients kennen keinen Preflight und liefen weiter — deshalb war die
Suite gruen, waehrend jeder Browser-Client ausgesperrt war.

Geprueft wird mit echten Anfragen gegen die zusammengebaute App. Ein Blick in
...
from fedlex_mcp.server import CORS_ROUTING_HEADERS, build_http_app, settings
...
def test_die_liste_nennt_die_header_die_das_sdk_liest() -> None:
    """Gegen die Konstanten des SDK gehalten, nicht gegen abgeschriebenen
    Spec-Text: `mcp.shared.inbound` ist das, womit der Server die Anfrage
...
async def test_kein_tool_schema_verlangt_einen_mcp_param_header() -> None:
    ...
    Header also einzeln freigeben. Bisher tut es keines — dieser Test ist die
    Erinnerung an dem Tag, an dem sich das aendert."""
    from fedlex_mcp.server import mcp
...
def test_die_methodenliste_nennt_die_sessionbeendigung() -> None:
    ...
    from fedlex_mcp.server import CORS_ALLOW_METHODS
